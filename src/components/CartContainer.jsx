...
const CartContainer = () => {
    ...
    const [{ user, cartShow, cartItems, foodItems, total }, dispatch] = useStateValue();
    ...
    const clearCart  = () => {
        ...
        localStorage.removeItem('cartItems'); //remove user from localStorage
    }
...

                        {
                            user ? (
                                <motion.button
                                    whileTap={{ scale: 0.8}}
                                    type='button'
                                    className='w-full p-2 rounded-full text-lg font-semibold my-2
                                    bg-gradient-to-tr from-orange-400 to-orange-600 text-gray-50  
